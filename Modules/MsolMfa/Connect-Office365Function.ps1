...
#>
	...
	Param (
		...
		[Alias('SPOrgName')]
		[string]$SharePointOrganizationName='nationalcinemedia',
	...
	If ($MFA -ne $True)
	{
        ...
        {
	        Throw "The Exchange Online MFA Module was not found!  https://docs.microsoft.com/en-us/powershell/exchange/exchange-online/connect-to-exchange-online-powershell/mfa-connect-to-exchange-online-powershell?view=exchange-ps"
        }			
        Else
        {
            If( -not $Credential ){  $Credential = Get-credential "$($env:USERNAME)@czadd.com" -Message "Enter your MSOL/Exchange credentials" }
        }
	}
...
#					Import-PSSession $Session -DisableNameChecking
					...
					If ($Null -ne (Get-PSSession | Where-Object { $_.ConfigurationName -like "*Exchange*" }))
					{
                        Write-Host "Connected to: Security and Compliance" -Foregroundcolor Green						
                        If (($host.ui.RawUI.WindowTitle) -notlike "*Connected To:*")
						{
							$host.ui.RawUI.WindowTitle += " - Connected To: Security and Compliance Center"
						}
			...
			SkypeForBusiness {
				...
				Else
				{
					...
					Else
					{
						...
						If ($Null -ne (Get-CsOnlineDirectoryTenant))
						{
							Write-Host "Connected to: Skype Online" -ForegroundColor Green
                            If (($host.ui.RawUI.WindowTitle) -notlike "*Connected To:*")
							{
								$host.ui.RawUI.WindowTitle += " - Connected To: Skype for Business"
							}
